...
async def advantage_spoll_choker(bot, query):
    ...
    if k == False:
        ...
        if files:
            ...
            k = await query.message.edit('𝚃𝙷𝙸𝚂 𝙼𝙾𝚅𝙸𝙴 I𝚂 𝙽𝙾𝚃 𝚈𝙴𝚃 𝚁𝙴𝙻𝙴𝙰𝚂𝙴𝙳 𝙾𝚁 𝙰𝙳𝙳𝙴𝙳 𝚃𝙾 𝙳𝙰𝚃𝚂𝙱𝙰𝚂𝙴.. 𝚁𝙴𝚀𝚄𝙴𝚂𝚃 𝙵𝙾𝚁 𝙰𝙳𝙼𝙸𝙽 😊 @RM_admin04bot 💌')
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("file"):
        ...
        buttons = [[
            ...
            InlineKeyboardButton('📢 UPDATES 📢', url='https://t.me/RM_STUDIOSZ')
            ],[      
        ...
        R1 = await query.message.reply_photo(
                 photo="https://te.legra.ph/file/16ee2b941190e67cf49ea.jpg",
...
async def auto_filter(client, msg, spoll=False):
    ...
    if imdb:
        ...
        cap = f"<b>🎬 Title:</b> {search}\n\n<b>👥 Requested by: {message.from_user.mention}</b>\n<b>© Powered by: <a href='https://t.me/+jNAQp5DRfd5kYTQ9'>{message.chat.title}</a></b>\n\n<b>✍️ Note:</b> <s>This message will be Auto-deleted after 5 minutes to avoid copyright issues.</s>"
    if imdb and imdb.get('poster'):
        try:
            hehe = await message.reply_photo(photo=imdb.get('poster'), caption=cap, reply_markup=InlineKeyboardMarkup(btn))
            await asyncio.sleep(IMDB_DELET_TIME)
            await hehe.delete()
            await message.reply_text(text=f"Your movie {search} Close", disable_notification = True)
        except (MediaEmpty, PhotoInvalidDimensions, WebpageMediaEmpty):
            pic = imdb.get('poster')
            poster = pic.replace('.jpg', "._V1_UX360.jpg")
            hmm = await message.reply_photo(photo=poster, caption=cap, reply_markup=InlineKeyboardMarkup(btn))
            await asyncio.sleep(IMDB_DELET_TIME)
            await hmm.edit_text(text=f"Your movie {search} Close", disable_notification = True)
        except Exception as e:
            logger.exception(e)
            ...
            await asyncio.sleep(IMDB_DELET_TIME)
            await fek.edit_text(text=f"Your movie {search} Close")
    else:
        ...
        await asyncio.sleep(IMDB_DELET_TIME)
        await fuk.delete()
        await message.reply_text(text=f"Your movie {search} Close")
